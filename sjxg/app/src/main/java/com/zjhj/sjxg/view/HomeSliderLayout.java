...
import com.zjhj.sjxg.util.CountDownTimerUtil;
import com.zjhj.sjxg.widget.LoopViewPager;
...
//                    int newPosition = position % sliderViewList.size();
    ...
    private class ImagePagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            //创建将要下载的图片的URI
            Uri imageUri = Uri.parse(TextUtils.isEmpty(imageViewList.get(position%imageViewList.size()).getPic())?"":imageViewList.get(position%imageViewList.size()).getPic());
            ImageRequest request = ImageRequestBuilder.newBuilderWithSource(imageUri)
                    .setResizeOptions(new ResizeOptions(DPUtil.dip2px(375), DPUtil.dip2px(208)))
                    .build();
            DraweeController controller = Fresco.newDraweeControllerBuilder()
