...
import com.zjhj.sjxg.widget.picker.ArrayWheelAdapter;
import com.zjhj.sjxg.widget.picker.ArrayWheelView;
import com.zjhj.sjxg.widget.picker.OnWheelChangedListener;
import com.zjhj.sjxg.widget.picker.WheelView;
...
//			provinceList = JSONArray.parseArray(object.getJSONArray("data").getJSONObject(0).getJSONArray("_child").toJSONString(), ProvinceModel.class);
			...
			if (null != mCurrentProviceName && !"".equals(mCurrentProviceName)) {
				...
				if (null != provinceList) {
					for (int i = 0; i < provinceList.size(); i++) {
						if (mCurrentProviceName.equals(provinceList.get(i).getName())) {
							Log.i("info", "设置省==>" + i);
							//							mViewProvince.setCurrentItem(i);
							provincePosition = i;
							for (int j = 0; j < provinceList.get(i).getChildren().size(); j++) {
								if (mCurrentCityName.equals(provinceList.get(i).getChildren().get(j).getName())) {
									Log.i("info", "设置市==>" + j);
									//									mViewCity.setCurrentItem(j);
									cityPosition = j;
									for (int k = 0; k < provinceList.get(i).getChildren().get(j).getChildren().size(); k++) {
										if (mCurrentDistrictName.equals(provinceList.get(i).getChildren().get(j).getChildren().get(k).getName())) {
											Log.i("info", "设置区===>" + k);
				...
				if (provinceList != null && !provinceList.isEmpty()) {
					...
					List<CityModel> cityList = provinceList.get(0).getChildren();
					if (cityList != null && !cityList.isEmpty()) {
						mCurrentCityName = cityList.get(0).getName();
						List<DistrictModel> districtList = cityList.get(0).getChildren();
			...
			for (int i = 0; i < provinceList.size(); i++) {
				...
				List<CityModel> cityList = provinceList.get(i).getChildren();
				String[] cityNames = new String[cityList.size()];
				for (int j = 0; j < cityList.size(); j++) {
					// 遍历省下面的所有市的数据
					cityNames[j] = cityList.get(j).getName();
					List<DistrictModel> districtList = cityList.get(j).getChildren();
	...
	public void onClick(View v) {
		...
		case R.id.confirm_layout:
			...
			lis.refreshPriorityUI(mCurrentProviceName,
					mCurrentCityName, mCurrentDistrictName,provinceList.get(mViewProvince.getCurrentItem()).getId(),
					provinceList.get(mViewProvince.getCurrentItem()).getChildren().get(mViewCity.getCurrentItem()).getId(),
					provinceList.get(mViewProvince.getCurrentItem()).getChildren().get(mViewCity.getCurrentItem()).getChildren().get(mViewDistrict.getCurrentItem()).getId());
